...
class TradeAccount(object):
    ...
    def get_total_asset(self, date_str):
        total = self.cash
        for symbol, quantity in self.holds.items():
            total += quantity * self.get_price_by_date_str(symbol, date_str)
        return total

    def get_net_value(self, date_str):
        return self.get_total_asset(date_str) / self.init_cash
    ...
    def buy(self, date_str, symbol, buy_cash=None):
        ...
        if self.holds.has_key(symbol):
            self.holds[symbol] += quantity
        else:
            self.holds[symbol] = quantity
    ...
    def sell(self, date_str, symbol, quantity=None):
        if quantity is None:
            quantity = self.holds[symbol]
        elif quantity > self.holds[symbol]:
    ...
    def action(self, trade_node):
        if trade_node.action == 'buy':
            ...
            self.sell(trade_node.date.strftime('%Y-%m-%d'), trade_node.symbol, self.holds[trade_node.symbol] * trade_node.percentage)
        else:
...
class TradeSimulation(object):
    ...
    @staticmethod
    def simuate(trade_nodes, start_date, end_date = datetime.datetime.today()):
        trade_account = TradeAccount(daily_data_cache = TradeSimulation.daily_data_cache )
        ...
        while i < len(dates) or j < len(trade_nodes):
            if j < len(trade_nodes) and dates[i] > trade_nodes[j].date:
                ...
                trade_account.action(trade_nodes[j])
                j += 1
            else:
                date_str = dates[i].strftime('%Y-%m-%d')
                print (date_str, trade_account.get_net_value(date_str))
                i += 1
...
if __name__ == '__main__':
    ...
    TradeSimulation.simuate(trade_nodes, datetime.datetime.strptime('2017-01-01', '%Y-%m-%d'))
